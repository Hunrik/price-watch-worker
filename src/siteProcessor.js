...
export default async (data) => {
  try {
    ...
    const [site, $] = Promise.all([getSite(url.hostname), getBody(url.href)])
    if (isProduct($, site)) {
      ...
      if (!price) throw new Error({err: 'Price not found', url: url.href})
      if (!name) throw new Error({err: 'Product name not found', url: url.href})
      const data = {
        oldPrice: price,
        url: url.href,
        productName: name,
